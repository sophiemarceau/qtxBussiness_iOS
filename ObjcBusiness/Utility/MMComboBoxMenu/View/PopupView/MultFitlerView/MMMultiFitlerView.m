...
- (void)popupViewFromSourceFrame:(CGRect)frame completion:(void (^ __nullable)(void))completion {
    ...
    self.frame = CGRectMake(0, kNavHeight+45, kScreenWidth, 0);
    [rootView addSubview:self];
    
    NSUInteger numberOftableViews = 0;
    
    if (self.item.numberOflayers == MMPopupViewTwolayers) {
        numberOftableViews = 2;
    }
    if (self.item.numberOflayers == MMPopupViewThreelayers) {
        numberOftableViews = 3;
    }
    if (self.item.numberOflayers == MMPopupViewTwolayersOrThirdlayers) {
        numberOftableViews = 3;
    }
    ...
    for (NSInteger index = 0; index < numberOftableViews; index ++) {
        ...
        tableView.backgroundColor = [UIColor colorWithHexString:CellBGColor];
        ...
        }else if(self.item.numberOflayers  == MMPopupViewTwolayers)
        {
    ...
    //出现的动画
    LeftCellWidth  = kScreenWidth/3;
//    (self.item.numberOflayers == MMPopupViewThreelayers)?kScreenWidth/3:100.0f;
    [UIView animateWithDuration:AnimationDuration animations:^{
        ...
        for (int index = 0; index < self.tableViewArrays.count; index ++) {
            ...
            }else if (self.item.numberOflayers == MMPopupViewThreelayers) {
                tableView.frame = CGRectMake(LeftCellWidth * index, 0, LeftCellWidth, self.height);
            }else {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (self.item.numberOflayers == MMPopupViewTwolayers) {
        ...
        cell.backgroundColor = [UIColor colorWithHexString:CellBGColor];
        [cell resetFrame];
        return cell;
    }else if (self.item.numberOflayers == MMPopupViewThreelayers) {
        MMLeftCell *cell = [tableView dequeueReusableCellWithIdentifier:MainCellID forIndexPath:indexPath];
        ...
        return cell;
    }else{//MMPopupViewTwolayersOrThirdlayers
        MMLeftCell *cell = [tableView dequeueReusableCellWithIdentifier:MainCellID forIndexPath:indexPath];
        switch (tableView.tag) {
            case 0:
                cell.item = self.item.childrenNodes[indexPath.row];
        ...
        case 0:{
            if (self.item.numberOflayers == MMPopupViewTwolayersOrThirdlayers){
                [UIView animateWithDuration:animationDuration animations:^{
                    ...
                    secondTableView.frame = CGRectMake(kScreenWidth/3, 0, kScreenWidth*1/3, self.height);
                ...
                [UIView animateWithDuration:animationDuration animations:^{
                    firstTableView.frame = CGRectMake(0, 0, kScreenWidth/3, self.height);
                    secondTableView.frame = CGRectMake(kScreenWidth/3, 0, kScreenWidth*2/3, self.height);
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    switch (tableView.tag) {
        case 0:{
            ...
            }else if(self.item.numberOflayers == MMPopupViewTwolayersOrThirdlayers){
                [self.tableViewArrays[0] reloadData];
                [self.tableViewArrays[1] reloadData];
                [self.tableViewArrays[2] reloadData]; 
            }else {
        ...
        case 1:{
            ...
            //添加
            if (self.item.numberOflayers == MMPopupViewTwolayersOrThirdlayers) {
                if (currentIndex.childrenNodes.count > 0) {
                    [self.selectedArray addObject:[MMSelectedPath pathWithFirstPath:selectdPath.firstPath secondPath:indexPath.row thirdPath:-1]];
                }else{
                    [self.selectedArray addObject:[MMSelectedPath pathWithFirstPath:selectdPath.firstPath secondPath:indexPath.row]];
                }
            }else{
                [self.selectedArray addObject:[MMSelectedPath pathWithFirstPath:selectdPath.firstPath secondPath:indexPath.row thirdPath:-1]];
            }
