...
- (void)reload {
    ...
    if ([self.dataSource respondsToSelector:@selector(comBoBoxView:infomationForColumn:)]) {
        for (NSUInteger i = 0; i < count; i ++) {
            ...
            if (count ==  2) {
                dropBox.titleLabel.frame = CGRectMake(0, 0, width, self.height);
                dropBox.titleLabel.textAlignment = NSTextAlignmentCenter;
                dropBox.arrow.image = [UIImage imageNamed:@"topbar_icon_down-1"];
                dropBox.arrow.frame = CGRectMake(width - 6*AUTO_SIZE_SCALE_X - 74*AUTO_SIZE_SCALE_X,
                                                 (self.height - 4*AUTO_SIZE_SCALE_X)/2  ,
                                                 6*AUTO_SIZE_SCALE_X ,
                                                 4*AUTO_SIZE_SCALE_X);
            }
...
////    [self.layer addSublayer:self.topLine];
    ...
    self.bottomLine.frame = CGRectMake(0, self.height - 1 , self.width, 0.5*AUTO_SIZE_SCALE_X);
    self.bottomLine.backgroundColor = [UIColor colorWithHexString:@"CECECE"].CGColor;
...
- (void)didTapDropDownBox:(MMDropDownBox *)dropDownBox atIndex:(NSUInteger)index {
    ...
    if (self.symbolArray.count) {
        ...
        CGRect menuFrame = CGRectMake(0, kNavHeight, self.frame.size.width, self.frame.size.height);
        [popupView popupViewFromSourceFrame:menuFrame completion:^{
           self.isAnimation = NO;
        }];
