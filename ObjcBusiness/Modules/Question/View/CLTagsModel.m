...
- (void)setTagsArray:(NSArray<NSDictionary *> *)tagsArray {
    ...
    for (int i = 0; i < _tagsArray.count; i ++) {
        ...
        currentTagBtn = [CLTagButton initWithTagDesc:[_tagsArray[i] objectForKey:@"tag_content"]];
        currentTagBtn.tag = [[_tagsArray[i] objectForKey:@"tag_id"] intValue];
...
- (UIView *)reloadTagViewPreTag:(UIView *)preTagBtn currentTagBtn:(UIView *)currentTagBtn {
    ...
    CGFloat preTaling = preTagBtn? CGRectGetMaxX(preTagBtn.frame) : 5;
    CGFloat preBottom = preTagBtn? CGRectGetMaxY(preTagBtn.frame) : 0;
    CGFloat preY = preTagBtn? preTagBtn.frame.origin.y : 0;
